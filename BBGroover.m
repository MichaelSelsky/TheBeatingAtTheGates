...
- (void) run:(NSNumber *)tick {
    ...
    NSLog(@"%lui", (unsigned long)_currentSubdivision);
    ...
    if (totalTicks == 0) {
        @throw [NSException exceptionWithName:@"Invalid Subdivision" reason:[NSString stringWithFormat:@"Subdivision %lui is too large to be expressed in %lui/%lui time.", (unsigned long)_currentSubdivision, (unsigned long)_groove.beats, (unsigned long)_groove.beatUnit] userInfo:nil];
